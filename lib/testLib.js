...
class testLib {
  ...
  async checkMintedAccounts(token, accounts, amounts) {
    ...
    for (let i = 0; i < accounts.length; i += 1) {
      new BigNumber(amounts[i]).should.be.bignumber.equal(balances[i], 'Wallet' + accounts[i] + ' balance is wrong')
    }
  }
  ...
  /**
   *  Check that assets are freezing before vesting time
   *
   * @param token - contract
  ...
  /**
   *  Check that assets are not freezing after vesting time
   *
   * @param token - contract
   ...
   */
  async buyAllTokens(customerAddress, crowdsale, saleGoalInWei, token, generalSaleWalletAddress) {
    const contractBalanceBefore = await web3.eth.getBalance(crowdsale.address);
    console.log('contractBalanceBefore by all', contractBalanceBefore.toNumber());
    let crowdsaleWalletBalance = 0;
    if (token && generalSaleWalletAddress) {
      console.log('generalSaleWalletAddress', generalSaleWalletAddress);
      crowdsaleWalletBalance = (await token.balanceOf(generalSaleWalletAddress));
      console.log('crowdsaleWalletBalance', crowdsaleWalletBalance.toNumber());
    }
    ...
    console.log('contractBalanceAfter by all', contractBalanceAfter.toNumber());
    if (token && generalSaleWalletAddress) {
      console.log('generalSaleWalletAddress', generalSaleWalletAddress);
      crowdsaleWalletBalance = (await token.balanceOf(generalSaleWalletAddress));
      console.log('crowdsaleWalletBalance', crowdsaleWalletBalance.toNumber());
  ...
  async checkCrowdsaleConditions(token, crowdsale, pools, goal) {
    ...
    for (let i = 0; i < pools.length; i += 1) {
      ...
      console.log('pools[i].address', pools[i].address,
        'addressBalance', addressBalance,
        'pools[i].allocationAmount * 10 ** tokenDecimals', pools[i].allocationAmount * 10 ** tokenDecimals
      );
    ...
    console.log('crowdsaleGoal', crowdsaleGoal);
  ...
  async checkIcoStageDates(crowdsale, preSaleStartDate, preSaleEndDate, generalSaleStartDate, generalSaleEndDate) {
    if (preSaleStartDate && preSaleEndDate) {
      const preSaleStartDateBE = (await crowdsale.preSaleStartDate()).toNumber();
      const preSaleEndDateBE = (await crowdsale.preSaleEndDate()).toNumber();
      // console.log('preSaleStartDateBE', preSaleStartDateBE, 'preSaleEndDateBE', preSaleEndDateBE);
      assert.equal(preSaleStartDate, preSaleStartDateBE, 'Pre-sale start date should be set accordingly to crowdsale conditions');
      assert.equal(preSaleEndDate, preSaleEndDateBE, 'Pre-sale end date should be set accordingly to crowdsale conditions');
    }
    const saleStartDate = (await crowdsale.saleStartDate()).toNumber();
    const saleEndDate = (await crowdsale.saleEndDate()).toNumber();
    // console.log('saleStartDate', saleStartDate, 'saleEndDate', saleEndDate);
    assert.equal(generalSaleStartDate, saleStartDate, 'General sale start date should be set accordingly to crowdsale conditions');
    assert.equal(generalSaleEndDate, saleEndDate, 'General sale end date should be set accordingly to crowdsale conditions');
  }
  ...
  async checkBuyTokens(crowdsale, token, sender, generalSaleWalletAddress, amountInWei, expectedSpentWei, expectedTokensAmount, expectedGoalReached)  {
    ...
    console.log('totalCollected', totalCollected.toNumber());
    await crowdsale.sendTransaction({ from: sender, to: crowdsale.address, value: amountInWei, gasLimit: gasLimit, gasPrice: gasPrice });

    const senderETHDiff = (senderBalanceETHBefore).minus(await web3.eth.getBalance(sender)).minus(this.getLatestBlockCost(gasPrice));
    ...
    const totalCollectedDiff = (await crowdsale.totalCollected());
    ...
    console.log('totalCollectedDiff = ', totalCollectedDiff.toNumber());

    senderETHDiff.should.be.bignumber.equal(new BigNumber(expectedSpentWei));
    ...
    contractTokensDiff.should.be.bignumber.equal(new BigNumber(expectedTokensAmount));
  ...
  async checkBuyAllTokens(crowdsale, token, sender, generalSaleWalletAddress, saleGoalInWei, tokenRate){
    ...
    if (token && generalSaleWalletAddress) {
      console.log('generalSaleWalletAddress', generalSaleWalletAddress);
      const crowdsaleWalletBalance = (await token.balanceOf(generalSaleWalletAddress));
      console.log('crowdsaleWalletBalance', crowdsaleWalletBalance.toNumber());
      console.log('amountToBuyInTokens', amountToBuyInTokens);
    }
  ...
  async checkWithdrawalIsDenied(crowdsale, address){
    const gasPrice = 100000;
    const gasLimit = 20e9;
    ...
    var withdrawalCall = async() => {
      await crowdsale.safeWithdrawal({from: address, gasLimit: gasLimit, gasPrice: gasPrice});
    };
  ...
  async checkWithdrawalIsAllowed(crowdsale, owner, bank){
    const gasPrice = 100000;
    const gasLimit = 20e9;
    const contractBalanceBefore = await web3.eth.getBalance(crowdsale.address);
    const bankBalanceBefore = await web3.eth.getBalance(bank.address);
    console.log('contractBalanceBefore = ', contractBalanceBefore.toNumber());

    await crowdsale.safeWithdrawal({from: owner, gasLimit: gasLimit, gasPrice: gasPrice});

    const contractBalanceAfter = await web3.eth.getBalance(crowdsale.address);
    console.log('contractBalanceAfter withdraw = ', contractBalanceAfter.toNumber());
    const bankBalanceAfter = await web3.eth.getBalance(bank.address);
    new BigNumber(contractBalanceAfter).should.be.bignumber.equal(0);
    bankBalanceBefore.should.be.bignumber.equal((bankBalanceAfter).minus(contractBalanceBefore));
  }

  async killCrowdsalePositive(crowdsale, token, generalSaleWalletAddress) {
    // save initial owner and contract balances (ETH)
    const owner = await crowdsale.owner();
    const initialOwnerBalance = await web3.eth.getBalance(owner);
    const contractBalance =  await web3.eth.getBalance(crowdsale.address);
    const isActive = await crowdsale.isICOActive();
    const contractBalanceBefore = await web3.eth.getBalance(crowdsale.address);
    console.log('contractBalanceBefore Positive Kill', contractBalanceBefore.toNumber());
  ...
  async killCrowdsaleNegative(crowdsale, address) {
    ...
    const contractBalanceBefore = await web3.eth.getBalance(crowdsale.address);
    console.log('contract balance BeforeKill', contractBalanceBefore.toNumber());
    const gasPrice = 1;
    var killCall = async() => {
