...
class testLib {
  ...
  async checkWithdrawalIsAllowed(crowdsale, address, amount, token, generalSaleAddress){
    const gasPrice = 100000;
    const gasLimit = 20e9;

    const contractBalance = await web3.eth.getBalance(crowdsale.address);
    const senderBalance = await web3.eth.getBalance(address);
    const generalSaleWalletBalance = await token.balanceOf(generalSaleAddress);
    ...
    console.log('generalSaleWalletBalance', generalSaleWalletBalance);
    await crowdsale.safeWithdrawal(amount, {from: address, gasLimit: gasLimit, gasPrice: gasPrice});
