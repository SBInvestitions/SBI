...
class testLib {
  ...
  async checkWithdrawalIsAllowed(crowdsale, owner, bank, amount){
    const gasPrice = 100000;
    const gasLimit = 20e9;
    ...
    await crowdsale.safeWithdrawal(amount, {from: owner, gasLimit: gasLimit, gasPrice: gasPrice});
    ...
    new BigNumber(contractBalanceBefore).should.be.bignumber.equal(new BigNumber(contractBalanceAfter).plus(amount));
    bankBalanceBefore.should.be.bignumber.equal((bankBalanceAfter).minus(amount));
  }
