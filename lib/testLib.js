...
class testLib {
  ...
  async checkCrowdsaleConditions(token, crowdsale, pools, goal) {
    ...
    for (let i = 0; i < pools.length; i += 1) {
      ...
      console.log('pools[i].address', pools[i].address, 'addressBalance', addressBalance, 'pools[i].allocationAmount * 10 ** tokenDecimals', pools[i].allocationAmount * 10 ** tokenDecimals);
