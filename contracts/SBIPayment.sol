...
/**
 * @title SBIPayment
 * @dev Base contract supporting async send for payments.
 * All payments available only after profit contribution
 */
contract SBIPayment {
  using SafeMath for uint256;
  // cunter for address payments
  mapping(address => uint32) public paymentCount;
  // amount of all payments
  uint256 public totalPayments = 0;
  // amount of contributions
  uint256 public totalContributionsAmount = 0;
  // contributions counter
  uint32 currentContributionCounter = 0;

  event NewIncomingFunds(uint indexed amount, address indexed sender);
  event NewWithdraw(uint indexed amount, address indexed payee);

  /**
   * @dev Constructor
   */
  function SBIPayment(address _tokenAddress) public payable {
  ...
  */
  function withdrawPayments() public {
    ...
    require(currentContributionCounter > paymentCount[payee]);
    require(this.balance > 0);
    require(token.balanceOf(payee) > 0);
    // total supply tokens is 40 000 000
    uint256 tokensPercent = token.balanceOf(msg.sender).div(40000000).div(1000000000000000);
    uint256 amountToWithdraw = tokensPercent.mul(this.balance).div(1000);
    assert(payee.send(amountToWithdraw));
    paymentCount[payee] = currentContributionCounter;
    totalPayments = totalPayments.add(amountToWithdraw);
    NewWithdraw(amountToWithdraw, payee);
  }
  ...
  function () external payable {
      ...
      totalContributionsAmount = totalContributionsAmount.add(amount);
      currentContributionCounter = currentContributionCounter.add(1);
  }
}